...
CREATE INDEX idx_signals_ticker_created ON trading_signals (ticker, created_at DESC);
CREATE INDEX idx_signals_source ON trading_signals (source, created_at DESC);
CREATE INDEX idx_signals_active ON trading_signals (executed, expires_at) WHERE NOT executed;
...
CREATE INDEX idx_positions_ticker ON positions (ticker, status);
...
CREATE INDEX idx_trades_ticker ON trades (ticker, exit_time DESC);
CREATE INDEX idx_trades_strategy ON trades (strategy, exit_time DESC);
...
CREATE INDEX idx_portfolio_snapshot_time ON portfolio_snapshots (snapshot_at DESC);
...
CREATE INDEX idx_candles_lookup ON candles (ticker, interval, open_time DESC);
